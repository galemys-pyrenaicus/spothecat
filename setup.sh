...
if VERB="$( which apt )" 2> /dev/null; then
  ...
  sudo dnf install -y git python3.6 glibc-locale-source glibc-langpack-ru
...
psql -h localhost -U postgres -c 'CREATE TABLE public.users (login text COLLATE pg_catalog."default" NOT NULL, pass_hash text COLLATE pg_catalog."default", role text COLLATE pg_catalog."default" NOT NULL, active boolean NOT NULL, address text COLLATE pg_catalog."default", connected text[] COLLATE pg_catalog."default", CONSTRAINT users_pkey PRIMARY KEY (id, login)) TABLESPACE pg_default;'
...
psql -h localhost -U postgres -c "INSERT INTO users (login, pass_hash, role, active) VALUES ('admin', '8C6976E5B5410415BDE908BD4DEE15DFB167A9C873FC4BB8A81F6F2AB448A918', 'admin', 'True')"

sudo cat > /lib/systemd/system/spothecat.service << EOL
[Unit]
